"""
Perform the recoding step based on a simple signal of the cross entropy of the gold token.
"""
...
class CrossEntropyRecoding(RecodingMechanism):
    """
    Define the error signal of the most simple recoding mechanism by calculating the cross-entropy error of the next
    token.
    """
    def __init__(self, model: AbstractRNN, predictor_kwargs: Dict, step_type: str, **unused: Any):
        ...
        self.ce_delta = CrossEntropyLoss().to(self.model.device)
    ...
    def recoding_func(self, input_var: Tensor, hidden: HiddenDict, out: Tensor, device: torch.device,
        ...
        delta = self.get_cross_entropy(out, target_idx)
    ...
    def get_cross_entropy(self, out: Tensor, target_idx: Optional[Tensor] = None) -> Tensor:
        out = out.squeeze(1)

        # If target indices are not given, just use most likely token
        if target_idx is None:
            target_idx = torch.argmax(out, dim=1)
        else:
            target_idx = target_idx

        delta = self.ce_delta(out, target_idx)
